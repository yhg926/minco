# KSSD3A-Based minco Staged Port Plan

Date: 2026-06-17

## Goal

Build a minco sketcher from the KSSD3A sketch path, then replace only
...
The current C++ minco compact BMI2 vector/radix backend improved 1000-genome
...
- Current minco: `bin/minco_compact_bmi2 sketch -p 8 -s 10000`
...
| minco compact old | 9.76 s | 195.4 MB | 80.16 MB | C++ map/heap |
| minco compact BMI2 vector/radix | 4.93 s | 349.7 MB | 80.16 MB | fixed-size MinHash, byte-identical to old compact |
...
- Command:
  `bin/minco_stage0 sketch -f8 -p8 -l /tmp/minco_1000_speed/genomes.list -o /tmp/kssd3a_based_minco_stages/stage0_kssd3a_f8`
- Wall time: 2.87 s
- User time: 21.68 s
- Max RSS: 243.7 MB
...
`ctxobj` keys to minco-style `(ctxhash44 << 20) | obj` keys and cap each
...
- Byte-identical to current minco compact output.
...
sets and compare ANI accuracy with the current minco compact output, KSSD3A,
skani, and Mash.

Expected invariant:

- If the MinHash selection is semantically the same as current minco compact,
  same-species ANI should match current minco compact.
...
`bin/minco_stage3_native`, which uses the copied KSSD3A parser/data path,
...
to the exact Stage 2 output and to the current minco compact sketch.

Compared with the previous best C++ minco compact BMI2/vector/radix sketcher
...
- KSSD3A `-f8` and minco fixed-size `-s10000` do not retain the same number
  of contexts, so speed is not a perfect apples-to-apples comparison until Stage
  3 isolates selection cost.
- minco compact stores context hashes rather than exact contexts, so rare hash
