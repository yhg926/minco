...
WORK="/tmp/kssd3a_based_minco_stages"
LIST="/tmp/minco_1000_speed/genomes.list"
...
# Baseline B: current minco best compact sketch.
/usr/bin/time -v "$ROOT/bin/minco_compact_bmi2" sketch -p 8 -s 10000 \
  -l "$LIST" \
  -o "$WORK/minco_compact_bmi2.kmini" \
  > "$WORK/minco_compact_bmi2.sketch.stdout" \
  2> "$WORK/minco_compact_bmi2.sketch.time.log"

wc -c "$WORK/minco_compact_bmi2.kmini" > "$WORK/minco_compact_bmi2.size.tsv"

# Stage 0: copied KSSD3A source path, unchanged fixed-rate selection.
/usr/bin/time -v "$ROOT/bin/minco_stage0" sketch -f8 -p8 \
  -l "$LIST" \
...
# Stage 1: KSSD3A -f8 prefilter retained, then minco hash-bottom-k cap.
/usr/bin/time -v "$ROOT/bin/minco_stage1" sketch -f8 -p8 \
  -l "$LIST" \
...
/usr/bin/time -v "$ROOT/bin/minco_stage2" sketch -f8 -p8 \
  -l "$LIST" \
...
/usr/bin/time -v "$ROOT/bin/minco_stage3_native" sketch -f8 -p8 \
  -l "$LIST" \
