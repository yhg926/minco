# Fixed-Size Context MinHash KSSDmini V1
...
Implemented `bin/kssdmini` in this repo.
...
- compact comparison binary `bin/kssdmini_compact` stores each retained item as
...
- BMI2 compact comparison binary `bin/kssdmini_compact_bmi2` writes the same
  compact sketch format but also uses KSSD3A-style `_pext_u64` context/object
  extraction on BMI2-capable x86 CPUs;
- rolling-hash comparison binary `bin/kssdmini_compact_roll` uses the same
...
- KSSDmini hybrid: MAE 0.496870 ANI percentage points, RMSE 0.508351,
  Pearson r 0.989031.
- Existing baseline summary: KSSD3A best MAE 0.498071; skani MAE 0.374781.
- Optimized KSSDmini `-p8` timing: sketch 0.16 s / 41.4 MB RSS, compare
  0.05 s / 22.1 MB RSS.
...
- KSSDmini hybrid overall: MAE 0.249972, RMSE 0.333985, Pearson r 0.969672.
- Existing baseline summary: KSSD3A best MAE 0.162096; skani MAE 0.177801.
- KSSDmini hybrid by species:
  Brucella 0.056981 MAE, E. coli 0.324455, Kingella 0.053644,
  Neisseria 0.496870, Pseudomonas_E 0.317910.
- Optimized KSSDmini `-p8` timing for 100 genomes: sketch 1.05 s / 87.5 MB
  RSS, all-vs-all compare 0.40 s / 98.1 MB RSS.
- Compact KSSDmini `-p8` timing for 100 genomes: sketch 0.98 s / 58.2 MB RSS,
...
- Rolling compact KSSDmini `-p8` timing for 100 genomes: sketch 1.22 s /
...
- KSSDmini full: sketch 10.59 s / 498.7 MB RSS, all-vs-all ANI 17.19 s /
  1095.5 MB RSS, total 27.78 s.
- KSSDmini compact mix64: sketch 9.76 s / 195.4 MB RSS, all-vs-all ANI
  12.29 s / 479.2 MB RSS, total 22.05 s.
- KSSDmini compact mix64 after lookup-table base decoding: sketch 9.27 s /
  193.9 MB RSS. The `.kmini` file was byte-identical to the regular compact
  mix64 baseline (`cmp` exit 0).
- KSSDmini compact BMI2 after lookup-table base decoding: sketch 7.37 s /
...
- KSSDmini compact vector/radix after porting KSSD3A-style gzip reading and
...
- KSSDmini compact BMI2 vector/radix: sketch 4.93 s / 349.7 MB RSS. The
  ...
  sketching.
- KSSDmini compact mix64 indexed detail: sketch 9.76 s / 195.4 MB RSS,
...
- KSSDmini compact mix64 indexed triangle: sketch 9.76 s / 195.4 MB RSS,
  triangle ANI 1.54 s / 247.8 MB RSS, total 11.30 s.
- KSSDmini compact rolling hash: sketch 11.68 s / 194.8 MB RSS, all-vs-all
...
- Output formats differ: KSSDmini writes a 1,000,000-pair full matrix plus
...
features already emitted by KSSDmini: `ANI_hybrid`, `ANI_obj`, `ANI_ctx_mash`,
