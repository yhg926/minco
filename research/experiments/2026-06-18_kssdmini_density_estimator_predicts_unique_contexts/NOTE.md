# KSSDmini density estimator predicts unique contexts
...
Can the KSSDmini density estimator from the final bottom-k threshold predict
...
- Input data: first 50 genomes from `/tmp/kssd3a_based_kssdmini_stages/genomes_50.list`.
...
```text
Normal estimator:
  bin/kssd3mini_stage3_native sketch -p 8 --ctxmeta both
  KSSDmini fixed sketch size = 10000 context hashes
...
Exploratory: KSSDmini can estimate per-genome total unique context cardinality
