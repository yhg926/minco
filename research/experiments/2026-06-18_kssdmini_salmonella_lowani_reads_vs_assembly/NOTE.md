# KSSDmini Salmonella Low-ANI Read-to-Assembly Check

Date: 2026-06-18

## Question

For a same-species Salmonella enterica pair below 99% ANI, does a KSSDmini sketch from real ONT reads estimate ANI to the other assembly close to the assembly-vs-assembly truth?
...
The FASTQ sketches were the existing 2009K-1545 KSSDmini sketches made with `--conflict --readsQC --ctxmeta both`.
...
KSSDmini assembly-vs-assembly ANI was `0.987683`, only `0.000094` below the `fastANI` truth.
...
For this below-99% same-species Salmonella pair, KSSDmini read-to-assembly estimates are close to assembly truth after moderate yield. The remaining high-yield bias is about `0.00028` ANI against `fastANI`, or `0.00018` ANI against KSSDmini's own assembly-vs-assembly estimate.

## Caveats

- Only one side has real ONT reads in this test.
- The assembly truth here is `fastANI`, not full ANIm.
- This uses KSSDmini `--readsQC`; no no-QC FASTQ control was rerun for this pair.
