...
SK_2009=/tmp/minco_asm_2009K1545_0618
SK_READS_2009=/tmp/minco_readsqc_kssdqcsample_counts_all_0618
SK_GTDB=/tmp/minco_asm_GCF001272115_0618
...
"$KSSD" ani -r "$SK_GTDB" --qraw "$SK_2009" -n0 -f0 -t0 -s4 -m0 \
  -o /tmp/minco_2009asm_vs_GCF001272115asm_0618.tsv

"$KSSD" ani -r "$SK_GTDB" --qraw "$SK_READS_2009" -n0 -f0 -t0 -s4 -m0 \
  -o /tmp/minco_2009reads_vs_GCF001272115asm_0618.tsv

"$KSSD" ani -r "$SK_READS_2009" --qraw "$SK_GTDB" -n0 -f0 -t0 -s4 -m0 \
  -o /tmp/minco_GCF001272115asm_vs_2009reads_0618.tsv
